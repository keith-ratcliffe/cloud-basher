...
CD_HADOOP_SERVICE_DIR="$( cd "$( dirname "${BASH_SOURCE[0]}" )" && pwd )"

CD_HADOOP_DIST_URI="http://archive-primary.cloudera.com/cdh5/cdh/5/hadoop-2.6.0-cdh5.9.1.tar.gz"
CD_HADOOP_DIST="$( downloadTarball "${CD_HADOOP_DIST_URI}" "${CD_HADOOP_SERVICE_DIR}" && echo "${tarball}" )"
CD_HADOOP_BASEDIR="$( getTarballBasedir "${CD_HADOOP_DIST}" "${CD_HADOOP_SERVICE_DIR}" && echo "${basedir}" )"
CD_HADOOP_SYMLINK="hadoop"

# core-site.xml (Format: <property-name><space><property-value>{<newline>})
CD_HADOOP_CORE_SITE_CONF="fs.defaultFS hdfs://localhost:9000
...
CD_HADOOP_HDFS_SITE_CONF="dfs.namenode.name.dir file://${CLOUD_DEVEL_DATA}/hadoop/nn
...
CD_HADOOP_MAPRED_SITE_CONF="mapreduce.jobhistory.address http://localhost:8020
...
CD_HADOOP_YARN_SITE_CONF="yarn.resourcemanager.scheduler.address localhost:8030
...
function hadoopUninstall() {
   if hadoopIsInstalled ; then
      ...
      rm -rf "${CD_HADOOP_SERVICE_DIR}/${CD_HADOOP_BASEDIR}"
...
function hadoopInstall() {
   "${CD_HADOOP_SERVICE_DIR}"/install.sh
}
